...
export function NavList({ data, sx, ...other }) {
  ...
  // For landing page navigation, only highlight exact matches
  // Hash links should not be considered active unless we're specifically on that section
  const isActive = pathname === '/' && data.path === '/' ? true : 
                   pathname === '/' && data.path.startsWith('/#') ? false :
